  ...
  -->
	<psxctl:ControlMeta name="sys_RadioButtons" dimension="single" choiceset="required">
		...
		<psxctl:ParamList>
			...
			<psxctl:Param name="helpHtml" datatype="String" paramtype="generic">
				<psxctl:Description>This parameter specifies the help text to be used on a control</psxctl:Description>
				<psxctl:DefaultValue>This is the default helpHtml</psxctl:DefaultValue>
			</psxctl:Param>
...
<!ATTLIST textarea
  ...
  -->
	...
	<xsl:template match="Control[@name='sys_CalendarSimple']" mode="psxcontrol">
		<input type="text" name="{@paramName}" value="{Value}" onblur="validateDatePattern(this);">
			<xsl:if test="@accessKey!=''">
				<xsl:attribute name="accesskey"><xsl:value-of select="@accessKey"/></xsl:attribute>
			</xsl:if>
			<xsl:call-template name="parametersToAttributes">
  ...
  -->
   ...
   <!-- 
	...
	<xsl:template match="Control[@name='sys_EditLive']" priority="10" mode="psxcontrol">
      ...
      <xsl:variable name="divname" select="concat($uniqueName,'_ps')"/>        
	  ...
	  <xsl:attribute name="onclick">_onclickEditLiveSection('<xsl:value-of select="$uniqueName"/>','<xsl:value-of select="$lang"/>',<xsl:value-of select="$readOnlyBoolean"/>)</xsl:attribute>
	  <xsl:attribute name="onfocus">javascript:this.onclick()</xsl:attribute>
      </div>
	...
	<!--  END Single Check Box -->
	 <!-- 
      ...
      <xsl:template match="Control[@name='sys_CheckBoxTree']" mode="psxcontrol-customcontrol-isdirty">		
	   ...
	   <xsl:template match="Control[@name='sys_CheckBoxTree']" mode="psxcontrol" priority="10">
	      <!-- set up the variables that will be used in the javascript -->
	      <xsl:variable name="name">
	         <xsl:value-of select="@paramName" />
	      </xsl:variable>
