...
<xsl:template match="/" />
<!--
	New Efficient Help Text Templates
-->
<xsl:template match="Control[ParamList/Param[@name='helpText']]" priority="25" mode="psxcontrol">
	...
	<!-- 
			override all controls to display the "helpText" or "helpHtml" parameter (if either exists), 
			then return control to the standard template
	-->
	<xsl:if test="ParamList/Param[@name='helpText'] or ParamList/Param[@name='helpHtml']">
		<!-- define both variables -->
		<xsl:variable name="helpText" select="ParamList/Param[@name='helpText']"/>
		<xsl:variable name="helpHtml" select="ParamList/Param[@name='helpHtml']"/>
		<!-- put a non breaking space before the helpText icon -->
		<xsl:text>&#160;</xsl:text>
		<!-- insert the helpText icon with the appropriate attributes -->
		<img src="../rx_resources/images/helpicon.gif" border="0" valign="center">
			<!-- only add the "alt" or "title" attributes if the "helpText" param is defined -->
			<xsl:if test="$helpText">
				<xsl:attribute name="alt"><xsl:value-of select="$helpText"/></xsl:attribute>
				<xsl:attribute name="title"><xsl:value-of select="$helpText"/></xsl:attribute>
			</xsl:if>
			<!-- only add the "onClick" attribute if the "helpHtml" param is defined -->
			<xsl:if test="$helpHtml">
				<xsl:attribute name="onClick">
					newWindow=window.open('','InlineHelp','width=500,height=250,resizable=yes');
					newWindow.document.write('<xsl:text disable-output-escaping="yes"><![CDATA[<html><head><title>Help Text PopUp</title></head><body>]]></xsl:text><xsl:value-of select="$helpHtml"/><xsl:text disable-output-escaping="yes"><![CDATA[</body></html>]]></xsl:text>');
